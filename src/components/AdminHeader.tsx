...
export function AdminHeader({ onRefresh, onExport, onClearDatabase, isRefreshing }: AdminHeaderProps) {
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (result.mode === 'push') {
          toast.success(`Notificación enviada a ${result.sent} dispositivo(s).${result.skipped ? ` Omitidos: ${result.skipped}.` : ''}`);
        } else {
          toast.success('Notificación local enviada (fallback).');
        }
      } else {
        if (!isPushNotificationSupported()) {
          toast.error('Tu navegador no soporta notificaciones push.');
        } else {
          toast.error(result.error ? `No se pudo enviar: ${result.error}` : 'No se pudo enviar la notificación.');
        }
      }
    } catch (error) {
      console.error(error);
